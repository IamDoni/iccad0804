...
float costFunc(vector<shared_ptr<Cell>>& mbff);
...
int main() {
	...
	STOP_TIMER(legal, "legalize()", logfile)
		...
		drawPlot();
	float cost = costFunc(FF);
	cout << "cost " << cost;

	FF.at(0)->updatePos(FF.at(0)->get_xpos(), FF.at(0)->get_ypos() + (float)100);
	cost = costFunc(FF);
	cout << "cost " << cost;
	...
	while (getline(file, line)) {
		...
		/////////////////////////////////////////////////////////////////
			...
			while (time--) {
				...
				auto temp_pin = make_shared<Pin>(tokens[1], stoi(tokens[2]), stoi(tokens[3]),"I");
		...
		/////////////////////////////////////////////////////////////////
			...
			while (time--) {
				...
				//Pin temp_pin = { tokens[1], stoi(tokens[2]), stoi(tokens[3]) };
				auto temp_pin = make_shared<Pin>(tokens[1], stoi(tokens[2]), stoi(tokens[3]),"O");
		...
		/////////////////////////////////////////////////////////////////
			...
			while (time--) {
				...
				// 创建一个 Pin 对象并填充数据
				auto temp_pin = std::make_shared<Pin>( tokens[1], std::stoi(tokens[2]), std::stoi(tokens[3]),"FG");
			...
			while (time--) {
				...
				for (auto& v : standard_FF) {
					if (v.get_ff_name() == tokens[2]) {
						...
						shared_ptr<Cell> temp_cell = make_shared<Cell>();
...
float costFunc(vector<shared_ptr<Cell>>& FF)
{
	...
	for (auto& reg : FF) {
		...
		float negative_slack = 0;

		for (auto& pin : reg->get_pin()) {
			if ((pin->getType() == 2) && (pin->get_timing_slack() < 0))  negative_slack -= pin->get_timing_slack();
		}

		cost += alpha * negative_slack;

		cout << "cost of negative slack = " << alpha<< " * " << negative_slack << endl;
		cout << "cost " << cost << endl;


		cost += Gamma * reg->get_ff_height() * reg->get_ff_width();

		cout << "cost of area = " << Gamma << " * " << reg->get_ff_height() * reg->get_ff_width() << endl;
		cout << "cost " << cost << endl;


		cost += beta * reg->get_power();
		cout << "cost of power = " << beta << " * " << reg->get_power() << endl;
		cout << "cost " << cost << endl;
